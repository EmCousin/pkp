...
describe 'Flash accessibility', type: :system do
  ...
  it 'keeps status messages available and restores focus after dismissal' do
    ...
    notice_button, warning_button = all('button[data-flash-dismiss]')

    page.execute_script('arguments[0].focus()', notice_button)
    press_enter

    expect(page).to have_no_text('Connecté·e.')
    expect(page.evaluate_script('document.activeElement === arguments[0]', warning_button)).to be(true)
  ...
  it 'uses an assertive alert for authentication errors' do
    ...
    dismiss_button = alert.find('button', text: 'Fermer')

    expect(dismiss_button[:'aria-describedby']).to eq('flash-alert-message')
    expect_component_to_be_accessible('#flash-messages')
  end
end
