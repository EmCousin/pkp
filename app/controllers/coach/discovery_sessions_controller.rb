...
module Coach
  class DiscoverySessionsController < BaseController
    ...
    def index
      @discovery_sessions = current_platform.discovery_sessions.active.recent.includes(:course, :subscriptions).order(:starts_at)
    end
    ...
    def set_discovery_session
      @discovery_session = current_platform.discovery_sessions.active.find(params[:id])
    end
  end
end
