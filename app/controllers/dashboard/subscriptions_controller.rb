...
module Dashboard
  class SubscriptionsController < DashboardController
    ...
    def create
      member = Current.user.members.find_by!(platform: Current.platform, id: subscription_params[:member_id])
    ...
    def set_subscription
      @subscription = Current.user.subscriptions.for_platform(Current.platform).not_archived.find_by(id: params[:id])
    end
    ...
    def available_members?
      Current.user.members.where(platform: Current.platform).available.any?
    end

    def set_member
      @member = Current.user.members.find_by(platform: Current.platform, id: params[:member_id])
    end
  end
end
