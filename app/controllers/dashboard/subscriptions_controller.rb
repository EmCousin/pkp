...
module Dashboard
  class SubscriptionsController < DashboardController
    ...
    def create
      member = current_user.members.where(platform: current_platform).find(subscription_params[:member_id])
    ...
    def set_subscription
      @subscription = current_user.subscriptions.for_platform(current_platform).not_archived.find_by(id: params[:id])
    end
    ...
    def available_members?
      current_user.members.where(platform: current_platform).available.any?
    end

    def set_member
      @member = current_user.members.where(platform: current_platform).find_by(id: params[:member_id])
    end
  end
end
