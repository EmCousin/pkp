...
class Invoice < ApplicationRecord
  ...
  def claim!(token)
    with_lock do
      return false unless sync_token == token
      return false unless pending? || retrying?
  ...
  def fail!(token, exception)
    with_lock do
      return false unless sync_token == token && (processing? || retrying?)
  ...
  def transition_owned_sync(token, attributes)
    with_lock do
      return false unless processing? && sync_token == token
