module ActiveRecord
    module HumanEnumName
      ...
      def human_enum_name(enum_name, enum_value)
        unless defined_enums.key?(enum_name.to_s)
          raise ArgumentError, "#{enum_name} is not a defined enum for #{self.model_name}"
        end
  
        unless defined_enums[enum_name.to_s].key?(enum_value)
          raise ArgumentError, "#{enum_value} is not a listed enum value for given enum #{enum_name}. Listed values are: #{defined_enums[enum_name.to_s].keys.to_sentence}"
        end
